...
#include "table/block_based_table_reader.h" // HEMANT
...
void DBImpl::PurgeObsoleteFiles(JobContext& state, bool schedule_only) {
  ...
  // HEMANT: map of subTable files used for storing columns
  std::map<uint64_t, std::vector<std::string>> subTableMap;
  ...
  for (auto& file : state.sst_delete_files) {
	std::string tableName = MakeTableFileName(kDumbDbName, file.metadata->fd.GetNumber());
    candidate_files.emplace_back(tableName, file.path);
    // HEMANT
    BlockBasedTable* reader = (BlockBasedTable*) file.metadata->fd.table_reader;
    if(reader->get_rep()->is_column_oriented){
    	std::vector<std::string> subTableList;
    	for(uint32_t i = 0; i < reader->get_rep()->tables.size(); i++) {
    		std::string subTableName = TableSubFileName(tableName, i+1);
    		subTableList.emplace_back(file.path + subTableName);
    	}
    	subTableMap.emplace(file.metadata->fd.GetNumber(), subTableList);
    }
    // HEMANT: end
  ...
  for (const auto& candidate_file : candidate_files) {
    ...
    if (schedule_only) {
      ...
      // HEMANT
      if(subTableMap.find(number) != subTableMap.end()) {
    	  for(std::string subFname : subTableMap[number]) {
    		  SchedulePendingPurge(subFname, dir_to_sync, type, number, state.job_id);
    	  }
      }
    } else {
	  ...
	  if(subTableMap.find(number) != subTableMap.end()) {
		  for(std::string subFname : subTableMap[number]) {
			  DeleteObsoleteFileImpl(state.job_id, subFname, dir_to_sync, type, number);
		  }
	  }
    }
  }
