...
void ForwardIterator::SeekInternal(const Slice& internal_key,
  ...
  if (seek_to_first || NeedToSeekImmutable(internal_key)) {
    ...
    const std::vector<FileMetaData*>& l0 = vstorage->LevelFiles(0,-1); // HEMANT: ForwardIterator is disabled. Figure this out if code hits here
    ...
    for (int32_t level = 1; level < vstorage->num_levels(); ++level) {
      const std::vector<FileMetaData*>& level_files =
          vstorage->LevelFiles(level,-1);  // HEMANT: ForwardIterator is disabled. Figure this out if code hits here
...
void ForwardIterator::RebuildIterators(bool refresh_sv) {
  ...
  const auto& l0_files = vstorage->LevelFiles(0,-1);	// HEMANT: ForwardIterator is disabled. Figure this out if code hits here
...
void ForwardIterator::RenewIterators() {
  ...
  const auto& l0_files_new = vstorage_new->LevelFiles(0,-1); // HEMANT: ForwardIterator is disabled. Figure this out if code hits here
  size_t iold, inew;
  bool found;
...
void ForwardIterator::BuildLevelIterators(const VersionStorageInfo* vstorage) {
  ...
  for (int32_t level = 1; level < vstorage->num_levels(); ++level) {
    const auto& level_files = vstorage->LevelFiles(level,-1);	// HEMANT: ForwardIterator is disabled. Figure this out if code hits here
    if ((level_files.empty()) ||
...
void ForwardIterator::ResetIncompleteIterators() {
  const auto& l0_files = sv_->current->storage_info()->LevelFiles(0,-1);	// HEMANT: ForwardIterator is disabled. Figure this out if code hits here
...
bool ForwardIterator::TEST_CheckDeletedIters(int* pdeleted_iters,
  ...
  for (int32_t level = 1; level < vstorage->num_levels(); ++level) {
    if ((level_iters_[level - 1] == nullptr) &&
        (!vstorage->LevelFiles(level,-1).empty())) {	// HEMANT: ForwardIterator is disabled. Figure this out if code hits here
      retval = true;
      deleted_iters++;
    } else if (!vstorage->LevelFiles(level,-1).empty()) { // HEMANT: ForwardIterator is disabled. Figure this out if code hits here
      num_iters++;
    }
  }
