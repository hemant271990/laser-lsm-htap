...
// HEMANT: TODO For now update indexer based on column 0 always. Fix this later
// file_indexer seems to be used by FilePicker whcih is used for Get() and RangeQuery() operation
void FileIndexer::UpdateIndex(Arena* arena, const size_t num_levels,
						std::vector<std::vector<FileMetaData*>>* const files) {
  if (files == nullptr) {
    return;
  }
  ...
  for (size_t level = 1; level < num_levels_ - 1; ++level) {
    const auto& upper_files = files[level][0];
    const int32_t upper_size = static_cast<int32_t>(upper_files.size());
    const auto& lower_files = files[level + 1][0];
  ...
  level_rb_[num_levels_ - 1] =
      static_cast<int32_t>(files[num_levels_ - 1][0].size()) - 1;
}
