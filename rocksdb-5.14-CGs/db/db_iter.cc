...
class DBIter final: public Iterator {
 public:
  ...
  DBIter(Env* _env, const ReadOptions& read_options,
      ...
      : arena_mode_(arena_mode),
        ...
        current_entry_is_column_merged(false),
        ...
        delim_(cf_options.delim),
        column_num_(cf_options.column_num) {
    RecordTick(statistics_, NO_ITERATORS);
    prefix_extractor_ = mutable_cf_options.prefix_extractor.get();
    max_skip_ = max_sequential_skip_in_iterations;
    max_skippable_internal_keys_ = read_options.max_skippable_internal_keys;
    filter_columns_ = read_options.filter_columns;
  ...
  virtual Slice value() const override {
    ...
    } else if (current_entry_is_column_merged) {  // HEMANT
      return saved_value_;
 ...
 private:
  ...
  bool MergeColumnValues(); // HEMANT
  bool MergeColumnValues2(std::string* result); // HEMANT
  void ApplyFilter(); // HEMANT
  ...
  bool current_entry_is_column_merged;  // HEMANT
  ...
  std::string delim_; // HEMANT
  int column_num_;  // HEMANT
  std::vector<uint32_t> filter_columns_;  // HEMANT: set the columns to be read via this
...
void DBIter::Next() {
  ...
  } else if (iter_->Valid() && !current_entry_is_merged_ && !current_entry_is_column_merged) {
  ...
  do {
    ...
    if (IsVisible(ikey_.sequence)) {
      if (skipping && user_comparator_->Compare(ikey_.user_key,
        ...
        num_skipped++;  // skip this entry when found another ikey_ for same saved_key_
        PERF_COUNTER_ADD(internal_key_skipped_count, 1);
      } else {
        num_skipped = 0;
        switch (ikey_.type) {
          ...
          case kTypeBlobIndex:
            if (start_seqnum_ > 0) {
              ...
              if (range_del_agg_.ShouldDelete(
                ...
                current_entry_is_column_merged = true;
                valid_ = true;
                return MergeColumnValues(); // HEMANT
              }
            }
            break;
          case kTypeMerge:
...
// HEMANT: keeps only the columns required by the filter
void DBIter::ApplyFilter() {
  if(filter_columns_.size() == 0) {
    return;
  }
  uint32_t curr_col = 1;
  uint32_t filter_idx = 0;
  uint32_t curr_col_size = 0;
  bool should_delete = false;
  uint32_t i = 0;
  while( i < saved_value_.size() ) {
    // check if needs to be deleted. Assumes filter_column_ is sorted increasing
    if(curr_col != filter_columns_[filter_idx]) {
      should_delete = true;
    }
    // collect the column value
    while(saved_value_[i] != delim_[0]) {
      curr_col_size++;
      i++;
    }
    if(should_delete) {
      // erase and decrease i by size + 1. +1 for the delim_
      saved_value_.erase(i - curr_col_size, curr_col_size+1);
      i = i - curr_col_size - 1;
    } else {
      // no need to delete this column, move to next filter column
      filter_idx++;
    }
    i++;
    curr_col_size = 0;
...
// HEMANT: Merge values of the same user key starting from the current iter_ position
// Scan from the newer entries to older entries.
// when column's value is found at two places, the earlier value is latest and should be saved.
// PRE: iter_->key() points to the first merge type entry
//      saved_key_ stores the user key
// POST: saved_value_ has the merged value for the user key
//       iter_ points to the next entry (or invalid)
bool DBIter::MergeColumnValues() {
  ClearSavedValue();
  std::vector<std::string> final_vals(filter_columns_.size());
  iter_->value().splitAndMergeTo(delim_[0], final_vals, filter_columns_);
  ...
  for (iter_->Next(); iter_->Valid(); iter_->Next()) {
    ...
    if (!user_comparator_->Equal(ikey.user_key, saved_key_.GetUserKey())) {
      ...
      // HEMANT: asserting false, because I don't expect other key types to reach this code
      // if they do, we can remove this assert
      assert(false);  
    }
  }
  if (!iter_->status().ok()) {
  ...
  for(size_t i = 0; i < final_vals.size(); i++) {
    saved_value_ += final_vals[i] + delim_;
  }
  return true;
}

// HEMANT: this is same as MergeColumnValues() but it used internall by MergeNewToOld()
// This method instead of assigning value to saved_value_ it returns a string of all
// column values combined. The above method is more optimized for the case when we don't
// encounter many merge keys, hence keeping two methods
// BELOW COMMENT IS SAME AS ABOVE METHOD:
...
// POST: saved_value_ has the merged value for the user key
//       iter_ points to the next entry (or invalid)
bool DBIter::MergeColumnValues2(std::string* result) {
  result->clear();
  std::vector<std::string> final_vals(column_num_);
  iter_->value().splitAndMergeTo(delim_[0], final_vals);
  ParsedInternalKey ikey;
  ...
  for(size_t i = 0; i < final_vals.size(); i++) {
    result->append(final_vals[i] + delim_);
  }
  return true;
}
...
bool DBIter::MergeValuesNewToOld() {
  ...
  for (iter_->Next(); iter_->Valid(); iter_->Next()) {
    ...
    if (!user_comparator_->Equal(ikey.user_key, saved_key_.GetUserKey())) {
      ...
      // HEMANT: collect all the column values for this put
      std::string val_str;
      MergeColumnValues2(&val_str);
      const Slice val(val_str);
      //const Slice val = iter_->value();
      // HEMANT: stats collection
      PERF_COUNTER_ADD(h2saxena_merge_for_scan, merge_context_.GetNumOperands());
      s = MergeHelper::TimedFullMerge(
          merge_operator_, ikey.user_key, &val, merge_context_.GetOperands(),
          &saved_value_, logger_, statistics_, env_, &pinned_value_, true);
      // HEMANT: only keep filter columns
      ApplyFilter();
      if (!s.ok()) {
        valid_ = false;
        status_ = s;
      ...
      // HEMANT: avoid this because the MergeColumnValues() would move the iter_ to next
      // iter_->Next();
      if (!iter_->status().ok()) {
        valid_ = false;
        return false;
