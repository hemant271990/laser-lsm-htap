...
UniversalCompactionPicker::CalculateSortedRuns(
  ...
  for (FileMetaData* f : vstorage.LevelFiles(0,-1)) {	// HEMANT: universal compaction will be disabled. Code should not hit here
  ...
  for (int level = 1; level < vstorage.num_levels(); level++) {
    ...
    bool is_first = true;
    for (FileMetaData* f : vstorage.LevelFiles(level,-1)) { // HEMANT: universal compaction will be disabled. Code should not hit here
...
Compaction* UniversalCompactionPicker::PickCompactionToReduceSortedRuns(
  ...
  for (size_t i = start_index; i < first_index_after; i++) {
    ...
    if (picking_sr.level == 0) {
      ...
      for (auto* f : vstorage->LevelFiles(picking_sr.level,-1)) {	// HEMANT: universal compaction will be disabled. Code should not hit here
        files.push_back(f);
      }
    }
    char file_num_buf[256];
...
Compaction* UniversalCompactionPicker::PickDeleteTriggeredCompaction(
  ...
  if (vstorage->num_levels() == 1) {
    ...
    for (output_level = start_level + 1; output_level < vstorage->num_levels();
         output_level++) {
      if (vstorage->NumLevelFiles(output_level,-1) != 0) {	// HEMANT: universal compaction will be disabled. Code should not hit here
        break;
      }
    }
  ...
  for (FileMetaData* f : vstorage->LevelFiles(output_level,-1)) {  // HEMANT: universal compaction will be disabled. Code should not hit here
