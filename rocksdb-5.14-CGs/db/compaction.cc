...
bool Compaction::IsBottommostLevel(
    ...
    const std::vector<CompactionInputFiles>& inputs, int col) { // HEMANT
  int output_l0_idx;
  if (output_level == 0) {
    output_l0_idx = 0;
    for (const auto* file : vstorage->LevelFiles(0,col)) { // HEMANT
      if (inputs[0].files.back() == file) {
        break;
      }
      ++output_l0_idx;
    }
    assert(static_cast<size_t>(output_l0_idx) < vstorage->LevelFiles(0,col).size()); // HEMANT
  } else {
    output_l0_idx = -1;
  }
  ...
  return !vstorage->RangeMightExistAfterSortedRun(smallest_key, largest_key,
                                                  output_level, output_l0_idx, col);  // HEMANT
}
...
bool Compaction::TEST_IsBottommostLevel(
  ...
  return IsBottommostLevel(output_level, vstorage, inputs, 0);	// HEMANT: Harcoded col=0 because its a test
}

bool Compaction::IsFullCompaction(
    VersionStorageInfo* vstorage,
    const std::vector<CompactionInputFiles>& inputs, int col) {
  size_t num_files_in_compaction = 0;
  size_t total_num_files = 0;
  for (int l = 0; l < vstorage->num_levels(); l++) {
	// HEMANT: compaction is full if all files for the given column are being compacted
    total_num_files += vstorage->NumLevelFiles(l,col);
  }
  for (size_t i = 0; i < inputs.size(); i++) {
...
Compaction::Compaction(VersionStorageInfo* vstorage,
    ...
    : input_vstorage_(vstorage),
      ...
      score_(_score),
      bottommost_level_(IsBottommostLevel(output_level_, vstorage, inputs_, col)),
      is_full_compaction_(IsFullCompaction(vstorage, inputs_, col)),
...
bool Compaction::IsTrivialMove() const {
  ...
  // HEMANT: UPDATE not a trivial move if num CGs change from star level to output level
  if(immutable_cf_options_.levels_cg_count.at(start_level_) < immutable_cf_options_.levels_cg_count.at(output_level_)) {
    return false;
  }
  // HEMANT: it is not a trivial move if output level is beyond k
  /* if(start_level_ < immutable_cf_options_.column_from_k &&
		  output_level_ >= immutable_cf_options_.column_from_k) {
	  return false;
  } */
  ...
  for (const auto& file : inputs_.front().files) {
    ...
    input_vstorage_->GetOverlappingInputs(output_level_ + 1, &file->smallest,
                                          &file->largest, &file_grand_parents, column_); // HEMANT
  ...
  if (bottommost_level_) {
    ...
    for (int lvl = output_level_ + 1; lvl < number_levels_; lvl++) {
      const std::vector<FileMetaData*>& files =
          input_vstorage_->LevelFiles(lvl,column_); // HEMANT:
