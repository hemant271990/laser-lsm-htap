...
bool CompactionPicker::ExpandInputsToCleanCut(const std::string& /*cf_name*/,
                                              ...
                                              CompactionInputFiles* inputs, int column) { // HEMANT
  ...
  do {
    ...
    inputs->clear();
    vstorage->GetOverlappingInputs(level, &smallest, &largest, &inputs->files, column,  // HEMANT
...
Status CompactionPicker::GetCompactionInputsFromFileNumbers(
  ...
  for (int level = 0; level < vstorage->num_levels(); ++level) {
	// HEMANT: This seems to be called for manual compaction. We won't support this. So col=-1 is fine
    for (auto file : vstorage->LevelFiles(level,-1)) {
...
bool CompactionPicker::IsRangeInCompaction(VersionStorageInfo* vstorage,
                                           ...
                                           int level, int* level_index, int col) {  // HEMANT
  std::vector<FileMetaData*> inputs;
  assert(level < NumberLevels());

  vstorage->GetOverlappingInputs(level, smallest, largest, &inputs, col,  // HEMANT
...
std::vector<std::tuple<uint32_t, uint32_t>> GetInputCGExpanded(int out_level, size_t in_level_cg, 
...
std::tuple<int, int> GetStartEndCGIdx(int out_level, std::vector<std::tuple<uint32_t, uint32_t>> cg_vec, 
                                      const std::vector<std::vector<std::tuple<uint32_t, uint32_t>>>& cg_range_matrix) {
  int start_cg_idx = -1;
  int end_cg_idx = -1;
  for (int i = 0; i < (int)cg_range_matrix[out_level].size(); i++) {
...
bool CompactionPicker::SetupOtherInputs(
  ...
  for (int l = input_level + 1; l < output_level; l++) {
    assert(vstorage->NumAllColumnLevelFiles(l) == 0);	// HEMANT
  }
  ...
  GetRange(*inputs, &smallest, &largest);
  // HEMANT: get column from the first file in start_level_inputs
  // At this point we know all files in start_level_inputs belong to the same column
  // and there is at least one file in it. So it is safe to do the below operation
  int col = inputs->files[0]->GetColumnNum();
  // Populate the set of next-level files (inputs_GetOutputLevelInputs()) to
  // include in compaction
  // HEMANT: if compaction is from k-1 to k, we should get overlapping inputs in k
  // for all columns. Loop over all columns and call GetOverlappingInputs()
  // UPDATE: if compaction is done on an expanding CG
  // then loop over all overlapping CGs in output level and call GetOverlappingInputs()
  std::vector<std::tuple<uint32_t, uint32_t>> cg_vec = GetInputCGExpanded(output_level, col, vstorage->cg_range_matrix());
  std::tuple<int, int> cg_start_end_idx_tuple = GetStartEndCGIdx(output_level, cg_vec, vstorage->cg_range_matrix());
  if (cg_vec.size() > 1) {
    for (int col_idx = std::get<0>(cg_start_end_idx_tuple); col_idx <= std::get<1>(cg_start_end_idx_tuple); col_idx++){
      CompactionInputFiles col_inputs;
      col_inputs.level = output_level;
      vstorage->GetOverlappingInputs(output_level, &smallest, &largest,
                                 &col_inputs.files, col_idx, *parent_index,
                                 parent_index);
      size_t old_size = col_inputs.files.size();
      if(!col_inputs.empty()) {
        if (!ExpandInputsToCleanCut(cf_name, vstorage, &col_inputs, col_idx)) {
          return false;
        }
      }
      size_t new_size = col_inputs.files.size();
      if (new_size > old_size) {
        ROCKS_LOG_INFO(ioptions_.info_log, " HEMANT: CleanCutExpansion was useful for column %d old_size %lu new_size %lu", col_idx, old_size, new_size);
      }
      for (size_t i = 0; i < col_inputs.files.size(); i++) {
        output_level_inputs->files.push_back(col_inputs.files[i]);
      }
    }
  } else {
    vstorage->GetOverlappingInputs(output_level, &smallest, &largest,
                                 &output_level_inputs->files, col, *parent_index,
                                 parent_index);
  }
  ...
  // HEMANT: if compaction is from k-1 to k, do ExpandInputsToCleanCut() when we are 
  // getting the overlapping inputs for each column, i.e. in line 426.
  // We do it early because output_level_inputs don't store files by column, it is a mixed bag of all columns
  // UPDATE: just do this when cg_vec.size() = 1
  if (cg_vec.size() == 1) {
    if (!output_level_inputs->empty()) {
      if (!ExpandInputsToCleanCut(cf_name, vstorage, output_level_inputs, col)) {
        return false;
      }
    }
  }
  ...
  // HEMANT: 27th Nov 2019, disabling expanding input level files.
  // observed a bug where the expanded input level expands out level too but
  // then don't include the newly added files of the output level in the compaction but 
  // keeps the newly added files of the input level, resulting into adding files with overlapping
  // keys after compaction. AddFile() assert(false) in version_set.cc was failing due to this.
  if ( false/* !output_level_inputs->empty() */) {
    ...
    vstorage->GetOverlappingInputs(input_level, &all_start, &all_limit,
                                   &expanded_inputs.files, col, base_index, nullptr);
    uint64_t expanded_inputs_size =
        TotalCompensatedFileSize(expanded_inputs.files);
    if (!ExpandInputsToCleanCut(cf_name, vstorage, &expanded_inputs, col)) {
    ...
    if (try_overlapping_inputs && expanded_inputs.size() > inputs->size() &&
      ...
      if (cg_vec.size() > 1) {
        for (int col_idx = std::get<0>(cg_start_end_idx_tuple); col_idx <= std::get<1>(cg_start_end_idx_tuple); col_idx++){
          ...
          for (size_t i = 0; i < col_inputs.files.size(); i++) {
            expanded_output_level_inputs.files.push_back(col_inputs.files[i]);
          }                        
        }
      } else {
        vstorage->GetOverlappingInputs(output_level, &new_start, &new_limit,
                                     &expanded_output_level_inputs.files, col,
                                     *parent_index, parent_index);
      }
      assert(!expanded_output_level_inputs.empty());
      // HEMANT: if compaction is from k-1 to k, do ExpandInputsToCleanCut() when we are 
      // getting the overlapping inputs for each column, i.e. in line 502.
      // We do it early because output_level_inputs don't store files by column, it is a mixed bag of all columns
      if (cg_vec.size() > 1) {
        ...
        if (!AreFilesInCompaction(expanded_output_level_inputs.files) &&
            ExpandInputsToCleanCut(cf_name, vstorage,
                                  &expanded_output_level_inputs, col) &&
            expanded_output_level_inputs.size() == output_level_inputs->size()) {
          expand_inputs = true;
        }
      }
...
void CompactionPicker::GetGrandparents(
  ...
  if (output_level_inputs.level + 1 < NumberLevels()) {
    ...
    // UPDATE: if compaction is done on an expanding CG
    // then loop over all overlapping CGs in output level and call GetOverlappingInputs()
    std::vector<std::tuple<uint32_t, uint32_t>> cg_vec = GetInputCGExpanded(output_level_inputs.level, col, vstorage->cg_range_matrix());
    std::tuple<int, int> cg_start_end_idx_tuple = GetStartEndCGIdx(output_level_inputs.level, cg_vec, vstorage->cg_range_matrix());
    if (cg_vec.size() > 1) {
      for (int col_idx = std::get<0>(cg_start_end_idx_tuple); col_idx <= std::get<1>(cg_start_end_idx_tuple); col_idx++){
        std::vector<FileMetaData*> col_inputs;
        vstorage->GetOverlappingInputs(output_level_inputs.level + 1, &start, &limit, &col_inputs, col_idx);
        for (size_t i = 0; i < col_inputs.size(); i++) {
          grandparents->push_back(col_inputs[i]);
        }                        
      }
    } else {
      vstorage->GetOverlappingInputs(output_level_inputs.level + 1, &start,
                                   &limit, grandparents, col);
    }
  }
}
...
Compaction* CompactionPicker::CompactRange(
  ...
  if (input_level == ColumnFamilyData::kCompactAllLevels) {
    ...
    *compaction_end = nullptr;
    ...
    int start_level = 0;
    // HEMANT: This seems to be called for manual compaction. We won't support this. So col=0 is fine
    for (; start_level < vstorage->num_levels() &&
           vstorage->NumLevelFiles(start_level,-1) == 0;
         start_level++) {
    }
    ...
    for (int level = start_level; level < vstorage->num_levels(); level++) {
      ...
      // HEMANT: This seems to be called for manual compaction. We won't support this. So col=0 is fine
      for (FileMetaData* f : vstorage->LevelFiles(level,-1)) {
        files.push_back(f);
      }
  ...
  // HEMANT: This seems to be called for manual compaction. We won't support this. So col=0 is fine
  if (ExpandInputsToCleanCut(cf_name, vstorage, &inputs, -1) == false) {
...
void CompactionPicker::PickFilesMarkedForCompaction(
  ...
  auto continuation = [&, cf_name](std::pair<int, FileMetaData*> level_file) {
    ...
    *output_level =
    ...
    return ExpandInputsToCleanCut(cf_name, vstorage, start_level_inputs, level_file.second->GetColumnNum()); // HEMANT: only one file in start_level_inputs, safe to do this
  };
...
bool CompactionPicker::GetOverlappingL0Files(
  ...
  // At this point we know all files in start_level_inputs belong to the same column
  // and there is at least one file in it. So it is safe to do the below operation
  int col = start_level_inputs->files[0]->GetColumnNum();
  // Note that the next call will discard the file we placed in
  // c->inputs_[0] earlier and replace it with an overlapping set
  // which will include the picked file.
  start_level_inputs->files.clear();
  vstorage->GetOverlappingInputs(0, &smallest, &largest,
                                 &(start_level_inputs->files), col);
...
class LevelCompactionBuilder {
 public:
  ...
  // HEMANT: function to check if all start_level_inputs and all output_level_inputs
  // belong to same column
  bool CheckAllSameColumn();
...
void LevelCompactionBuilder::PickExpiredTtlFiles() {
  ...
  auto continuation = [&](std::pair<int, FileMetaData*> level_file) {
    ...
    return compaction_picker_->ExpandInputsToCleanCut(cf_name_, vstorage_,
                                                      &start_level_inputs_,
													  level_file.second->GetColumnNum());	// HEMANT: level_file is the only file, get col from it
  };
  ...
  if (start_level_inputs_.empty()) {
    ...
    for (i = 0; i < vstorage_->BottommostFilesMarkedForCompaction().size();
      ...
      if (compaction_picker_->ExpandInputsToCleanCut(cf_name_, vstorage_,
                                                     &start_level_inputs_,
													 level_and_file.second->GetColumnNum())) { // HEMANT: level_and_file is the only files, so get colu from it
        break;
      }
    }
...
// HEMANT: all files should belong to same column
bool LevelCompactionBuilder::CheckAllSameColumn() {
	for (int i = 0; i < static_cast<int>(start_level_inputs_.files.size())-1; i++) {
		if (start_level_inputs_.files[i]->GetColumnNum() != start_level_inputs_.files[i+1]->GetColumnNum()) {
			return false;
		}
	}

	for (int i = 0; i < static_cast<int>(output_level_inputs_.files.size())-1; i++) {
		if (output_level_inputs_.files[i]->GetColumnNum() != output_level_inputs_.files[i+1]->GetColumnNum()) {
			return false;
		}
	}
	return true;
...
Compaction* LevelCompactionBuilder::PickCompaction() {
  ...
  assert(CheckAllSameColumn()); // HEMANT: All should be same column
...
Compaction* LevelCompactionBuilder::GetCompaction() {
  // HEMANT: pass column for later use. Always get it from start_level_inputs_
	int col = start_level_inputs_.files[0]->GetColumnNum();

  // UPDATE: use cg_vec to see if it is not an expanding compaction. If not then all inputs should belong to the same column
  if (!output_level_inputs_.files.empty()) {
    std::vector<std::tuple<uint32_t, uint32_t>> cg_vec = GetInputCGExpanded(output_level_inputs_.level, col, vstorage_->cg_range_matrix());
    if(cg_vec.size() == 1) {
      assert(CheckAllSameColumn()); // HEMANT: All should be same column
    }
  } else {
  ...
  auto c = new Compaction(
      ...
      start_level_score_, false /* deletion_compaction */, compaction_reason_, col);
...
bool LevelCompactionBuilder::PickFileToCompact() {
  ...
  for (cmp_idx = vstorage_->NextCompactionIndex(start_level_);
    ...
    int index = static_cast<int>(file_size[cmp_idx].index);
    int col = file_size[cmp_idx].file->GetColumnNum();
    const std::vector<FileMetaData*>& level_files =
          vstorage_->LevelFiles(start_level_,col);  // HEMANT: moved it inside the loop because col is not known until index is fixed.
    

    // HEMANT: add more files to compaction if in column levels
    // UPDATE: this is a heuristic to fasten compaction for CGs, 
    // pick more work when num CGs is more than 10 (random choice)
    if(vstorage_->levels_cg_count().at(start_level_) >= 5 ) {
      // HEMANT: max_start_level_input_count limits the number of input files in compaction.
      // Previous heuristic was creating massive compaction jobs, reducing parallelism,
      // and also moving all the data for a specific column to the next level, disrupting lifecycle based data distribution
      int max_start_level_input_count = 5;
      for (size_t i = 0; i < level_files.size(); i++) {
        auto* f = level_files[i];
        if (f->being_compacted) {
          continue;
        }
	if (max_start_level_input_count > 0) {
          start_level_inputs_.files.push_back(f);
	  max_start_level_input_count--;
	}
      }
      if(start_level_inputs_.empty()) {
        continue;
    ...
    start_level_inputs_.level = start_level_;
    if (!compaction_picker_->ExpandInputsToCleanCut(cf_name_, vstorage_,
                                                    &start_level_inputs_, col) ||	// HEMANT
    ...
    vstorage_->GetOverlappingInputs(output_level_, &smallest, &largest,
                                    &output_level_inputs.files, col); // HEMANT
    if (!output_level_inputs.empty() &&
        !compaction_picker_->ExpandInputsToCleanCut(cf_name_, vstorage_,
                                                    &output_level_inputs, col)) { // HEMANT
      start_level_inputs_.clear();
      continue;
    }
    base_index_ = index;
...
bool LevelCompactionBuilder::PickIntraL0Compaction() {
  start_level_inputs_.clear();
  // HEMANT: with columns, pick the column with most L0 files to compact
  int picked_col = 0;
  int max_file_count = 0;
  for (int col = 0; col < vstorage_->NumLevelColumns(0); col++) {
	  if (vstorage_->NumLevelFiles(0, col) >= max_file_count) {
		  picked_col = col;
		  max_file_count = vstorage_->NumLevelFiles(0, col);
	  }
  }
  const std::vector<FileMetaData*>& level_files =
      vstorage_->LevelFiles(0 /* level */,picked_col);  // HEMANT
  if (level_files.size() <
          static_cast<size_t>(
...
Compaction* FIFOCompactionPicker::PickTTLCompaction(
  ...
  const int kLevel0 = 0;
  // HEMANT: We won't support FIFO compaction. So col=0 is fine.
  const std::vector<FileMetaData*>& level_files = vstorage->LevelFiles(kLevel0,0);
