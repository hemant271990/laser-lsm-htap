...
class Slice {
 public:
  ...
  // HEMANT: split slice by delim
  std::vector<std::string> split(char delim) const;
  void split(char delim, std::vector<std::string>& splits) const;
  void splitByCGs(char delim, std::vector<std::string>& splits, const std::vector<std::tuple<uint32_t, uint32_t>>& CG) const;
  void splitAndMergeTo(char delim, std::vector<std::string>& vec, std::vector<uint32_t>& filter_columns) const;
  void splitAndMergeTo(char delim, std::vector<std::string>& vec) const;
...
class PinnableSlice : public Slice, public Cleanable {
 public:
  ...
  // HEMANT: used for Get() to append column values one after the other
  inline void UpdateSelf(const Slice& slice) {
    assert(!pinned_);
    buf_->append(1, '|');
    buf_->append(slice.data(), slice.size());
    data_ = buf_->data();
    size_ = buf_->size();
    assert(!pinned_);
  }
