...
struct Range {
  Slice start;
  Slice limit;
  SequenceNumber seq;   // HEMANT: Seq for range query

  static const SequenceNumber kMaxSequenceNumber = ((0x1ull << 56) - 1);

  Range() : seq(kMaxSequenceNumber) { } // HEMANT
  Range(const Slice& s, const Slice& l,
		  SequenceNumber sn = kMaxSequenceNumber) : start(s), limit(l), seq(sn) { } // HEMANT
};
...
/************************ HEMANT **************************/
using filterFun = bool(*)(std::map<uint32_t, const std::string*>&);
struct SeqTypeVal;
