...
struct DBOptions {
  ...
  int column_from_k = 1; // HEMANT: this level onwards it is column layout
  std::vector<size_t> col_attrs_offset = {}; // HEMANT: Vector to hold the offset value of each attribute
  std::vector<size_t> col_attrs_size = {}; // HEMANT: vector to hold the size of each attribute
...
struct ReadOptions {
  ...
  // HEMANT
  std::vector<uint32_t> filter_columns;  //  set the columns to be read via this
  std::vector<char> filter_types; //  types for filter. "D" for Double, and so on
  std::vector<std::string> filter_values;  //  filter value. E.g. 0.01
  std::vector<std::string> filter_comparator; //  comparator ">" or "<" or ">=" and so on
  bool unique_key;
  // HEMAN: END
