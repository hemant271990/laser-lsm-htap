...
// HEMANT
std::vector<std::string> StringSplitByOffset(const char* arg, std::vector<size_t> col_attrs_offset, std::vector<size_t> col_attrs_size) {
  std::vector<std::string> splits;
  for(uint32_t i = 0; i < col_attrs_offset.size(); i++) {
	  size_t attr_size = col_attrs_size[i];
	  char item[attr_size+1];
	  item[attr_size] = '\0';
	  memcpy(item, &arg[col_attrs_offset[i]], attr_size);
	  splits.push_back(std::string(item));
  }
  return splits;
}

std::vector<std::string> StringSplitWithGaps(const char* arg, char delim, std::vector<size_t> col_attrs_size) {
  std::vector<std::string> splits;
  std::stringstream ss(arg);
  std::string item;
  int i = 0;
  while (std::getline(ss, item, delim)) {
    if (item.size() > col_attrs_size[i]) {
      size_t extra = item.size() - col_attrs_size[i];
      size_t pos = col_attrs_size[i];
      item.erase(pos, extra);
    }
    splits.push_back(item);
    i++;
  }
