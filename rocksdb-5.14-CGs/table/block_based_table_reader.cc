...
Status BlockBasedTable::Open(const ImmutableCFOptions& ioptions,
                             ...
                             bool is_column_oriented_, // HEMANT
                             const std::vector<uint32_t>& cols) { // HEMANT
  table_reader->reset();
  ...
  rep->is_column_oriented = is_column_oriented_; // HEMANT
  rep->level = level; // HEMANT: (UPDATE for CGs)
  ...
  if(rep->is_column_oriented) {
	  ...
	  for (auto i = 0u; i < column_num; i++) {
		...
		col_file->SetFileName(col_fname);
		if (readahead > 0) {
		  col_file = NewReadaheadRandomAccessFile(std::move(col_file), readahead);
		}
		unique_ptr<RandomAccessFileReader> file_reader(
			new RandomAccessFileReader(std::move(col_file), col_fname, ioptions.env));
		unique_ptr<TableReader> table;
		// Get file size
		struct stat stat_buf;
		int rc = stat(col_fname.c_str(), &stat_buf);
		uint64_t col_file_size = -1;
		if(rc == 0) {
			col_file_size = stat_buf.st_size;
		}

		s = Open(ioptions, env_options, table_options, internal_comparator,
			   std::move(file_reader), col_file_size, &table,
			   prefix_extractor, prefetch_index_and_filter_in_cache,
			   skip_filters, level);
		if (!s.ok()) {
		  return s;
		}
		rep->tables[i].reset(static_cast<BlockBasedTable*>(table.release()));
	  }
  }
  // HEMANT: END
...
// HEMANT
void BlockBasedTable::NewIndexIterators(const ReadOptions& read_options,
  ...
  for (auto i = 0u; i < column_num; i++) {
    input_iter.SetIter(i, rep_->tables[i/*read_options.columns[i]*/]
                          ->NewIndexIterator(read_options, disable_prefix_seek,
                        		  	  	  	  nullptr, nullptr, get_context));
  }
}
// HEMANT: END
...
// HEMANT
void BlockBasedTable::NewDataBlockIterators(Rep* rep, const ReadOptions& read_options,
  ...
  for (auto i = 0u; i < column_num; i++) {
    input_iter.SetIter(i,
        NewDataBlockIterator(rep->tables[i/*read_options.columns[i]*/]->rep_,
...
// ---- BEGIN BlockBasedTableIterator ----

// HEMANT
Status BlockBasedTableIterator::RangeQuery(const LookupRange& range,
                            /*std::map<std::string, SeqTypeVal>&*/ int* res/*,
                            filterFun filter*/) {

	SequenceNumber sequence_num = range.SequenceNum();
	std::string user_key, val;
	Slice target = range.start_->internal_key();
	Slice limit = range.limit_->internal_key();
	if (!CheckPrefixMayMatch(target, prefix_extractor_)) {
		ResetDataIter();
		return status();
	}
	int count = 0;
	for(Seek(target); Valid(); Next()) {
		if (icomp_.Compare(key(), limit) >= 0) {
			break;
		}
		...
		if (parsed_key.sequence <= sequence_num && parsed_key.type != kTypeMerge) {
			if(read_options_.filter_values.size() > 0) {
				bool filter_pass = true;
				for(uint32_t j = 0; j < read_options_.filter_columns.size(); j++) {
					uint32_t col_index = read_options_.filter_columns[j]-1;
					size_t attr_size = table_->get_rep()->ioptions.col_attrs_size[col_index];
					size_t attr_offset = table_->get_rep()->ioptions.col_attrs_offset[col_index];
					char type = read_options_.filter_types[j];
					char c_val[attr_size];
					memcpy(c_val, &value().data_[attr_offset], attr_size);
					switch(type) {
						case 'D' :
						{
						  double filter = std::stod(read_options_.filter_values[j]);
						  double value = std::atof(c_val);
						  if(read_options_.filter_comparator[j] == ">") {
							  if(value <= filter)
								  filter_pass = false;
						  }
						  break;
...
/*Status BlockBasedTableIterator::RangeQuery(const LookupRange& range,
	...
	std::string user_key, val;
	std::unique_ptr<InternalIterator> blockhandles_iter(table_->NewIndexIterator(read_options_));
	Status s = blockhandles_iter->status();
	if (!s.ok()) {
		return s;
	}
	std::vector<BlockHandle> blck_size_vector;
	for (blockhandles_iter->SeekToFirst(); blockhandles_iter->Valid();
		...
		data_block_handle.DecodeFrom(&bh_val);
		blck_size_vector.emplace_back(data_block_handle);
		//Slice bh_val = blockhandles_iter->value();
		//BlockHandle data_block_handle;
		//data_block_handle.DecodeFrom(&bh_val);
		//if (data_block_handle.offset() +
		//	static_cast<size_t>(data_block_handle.size()) +
		//	kBlockTrailerSize > readahead_limit_) {
		//	readahead_size_ = kMaxReadaheadSize;
		//	table_->get_rep()->file->Prefetch(data_block_handle.offset(), readahead_size_);
		//	readahead_limit_ = static_cast<size_t>(data_block_handle.offset() + readahead_size_);
		//}

	}
	for(uint64_t i = 0; i < blck_size_vector.size(); i++) {
		Slice compression_dict;
		std::unique_ptr<Block> block_value;
		s = ReadBlockFromFile(
			  table_->get_rep()->file.get(), nullptr, table_->get_rep()->footer, read_options_,
			  blck_size_vector[i], &block_value, table_->get_rep()->ioptions, table_->get_rep()->blocks_maybe_compressed,
			  compression_dict, table_->get_rep()->persistent_cache_options,
			  is_index_ ? kDisableGlobalSequenceNumber : table_->get_rep()->global_seqno,
			  table_->get_rep()->table_options.read_amp_bytes_per_bit);

		BlockIter* datablock_iter;
		datablock_iter = new BlockIter;
		datablock_iter = block_value.get()->NewIterator(&icomp_, icomp_.user_comparator(),
				datablock_iter, true, table_->get_rep()->ioptions.statistics, key_includes_seq_);
		for (datablock_iter->SeekToFirst(); datablock_iter->Valid(); datablock_iter->Next()) {
			...
			ParsedInternalKey parsed_key;
			if (!ParseInternalKey(datablock_iter->key(), &parsed_key)) {
			  return Status::Corruption("corrupted internal key in Table::Iter");
			}
			...
			for(uint32_t j = 0; j < read_options_.filter_columns.size(); j++) {
				...
				char type = read_options_.filter_types[j];
				char c_val[attr_size];
				memcpy(c_val, &datablock_iter->value().data_[attr_offset], attr_size);
				switch(type) {
					case 'D' :
					{
					  double filter = std::stod(read_options_.filter_values[j]);
			...
			//user_key.assign(datablock_iter->key().data(), datablock_iter->key().size() - 8);
			//val.assign(datablock_iter->value().data(), datablock_iter->value().size());
			//res.emplace(user_key, SeqTypeVal(parsed_key.sequence, parsed_key.type, val));
		}
	}
	return Status();
...
InternalIterator* BlockBasedTable::NewIterator(
  ...
  // HEMANT: Do a different new iterator if column oriented
  if(rep_->is_column_oriented) {
	  std::vector<InternalIterator*> iters;
	  uint32_t column_num = rep_->ioptions.column_num;
	  if (arena == nullptr) {
		  for (auto i = 0u; i < column_num; i++) {
			  if (!read_options.filter_columns.empty() && std::find(read_options.filter_columns.begin(), read_options.filter_columns.end(), i+1) == read_options.filter_columns.end()) {
				  continue;
			  }
			  iters.push_back(new BlockBasedTableIterator(rep_->tables[i].get(),
														  read_options,
														  rep_->internal_comparator,
														  rep_->tables[i]->NewIndexIterator(
															read_options,
		  ...
		  for (auto i = 0u; i < column_num; i++) {
			  ...
			  iters.push_back(new (mem) BlockBasedTableIterator(rep_->tables[i].get(),
					  	  	  	  	  	  	  	  	  	  	  	read_options,
					  	  	  	  	  	  	  	  	  	  	  	rep_->internal_comparator,
					  	  	  	  	  	  	  	  	  	  	  	rep_->tables[i]->NewIndexIterator(read_options,
					  	  	  	  	  	  	  	  	  	  	  					 prefix_extractor_changed),
					  	  	  	  	  	  	  	  	  	  	  	!skip_filters && !read_options.total_order_seek &&
					  	  	  	  	  	  	  	  	  	  	 		prefix_extractor != nullptr && !prefix_extractor_changed,
					  	  	  	  	  	  	  	  	  	  	 	prefix_extractor,
	  ...
	  return new ColumnIterator(iters,
			  	  	  	  	  	rep_->ioptions.delim.at(0),
	                            rep_->internal_comparator,
	                            rep_->table_properties->num_entries,
	                            read_options.filter_columns,
	                            read_options.filter_types,
	                            read_options.filter_values,
	                            read_options.filter_comparator,
	                            rep_->ioptions.col_attrs_offset,
	                            rep_->ioptions.col_attrs_size);
  } else {
	  if (arena == nullptr) {
		return new BlockBasedTableIterator(
				// "this" is rep_->table[i]
...
Status BlockBasedTable::Get(const ReadOptions& read_options, const Slice& key,
                            ...
                            bool skip_filters) {
	if(rep_->is_column_oriented) {
		return GetColumnOriented(read_options, key, get_context, prefix_extractor, skip_filters);
	} else {
		return GetOriginal(read_options, key, get_context, prefix_extractor, skip_filters);
	}
}

Status BlockBasedTable::GetOriginal(const ReadOptions& read_options, const Slice& key,
                            GetContext* get_context,
                            const SliceTransform* prefix_extractor,
                            bool skip_filters) {
  ...
  Status s;
  const bool no_io = read_options.read_tier == kBlockCacheTier;
  // HEMANT: get column number from file number, needed for SaveValue()
  int col_num = TableFileNameToColumnNumber(rep_->file.get()->file_name());
      ...
      if (not_exist_in_filter) {
        ...
        for (biter.Seek(key); biter.Valid(); biter.Next()) {
          ...
          if (!get_context->SaveValue(
                  parsed_key, biter.value(), &matched, col_num, rep_->level,
...
Status BlockBasedTable::GetColumnOriented(const ReadOptions& read_options, const Slice& key,
                            GetContext* get_context,
                            const SliceTransform* prefix_extractor,
                            bool skip_filters) {
  assert(key.size() >= 8);  // key must be internal key
  ...
  if (!FullFilterKeyMayMatch(read_options, filter, key, no_io,
	...
	uint32_t column_num = rep_->ioptions.column_num;
	std::vector<InternalIterator*> v2(column_num /*read_options.columns.size()*/);
	ColumnIterator iiter(v2, rep_->ioptions.delim.at(0), rep_->internal_comparator);
	NewIndexIterators(read_options, prefix_extractor_changed, get_context, iiter, column_num);
	...
	bool done = false;
	for (iiter.Seek(key); iiter.Valid() && !done; iiter.Next()) {
	  //Slice handle_value = iiter->GetVals(); // HEMANT
	  //BlockHandle handle;
	  bool not_exist_in_filter = false;
		  /*filter != nullptr && filter->IsBlockBased() == true &&
		  handle.DecodeFrom(&handle_value).ok() &&
		  !filter->KeyMayMatch(ExtractUserKey(key), prefix_extractor,
							   handle.offset(), no_io);*/
	  ...
	  if (not_exist_in_filter) {
		...
		std::vector<InternalIterator*> v(column_num /*read_options.columns.size()*/);
		ColumnIterator biter(v, rep_->ioptions.delim.at(0), rep_->internal_comparator);
		NewDataBlockIterators(rep_, read_options, iiter.GetVals(), biter, column_num);
		...
		for (biter.Seek(key); biter.Valid(); biter.Next()) {
		  ...
		  if (!get_context->SaveValue(
				  parsed_key, biter.value(), &matched, -1, rep_->level,
				  biter.IsValuePinned() ? &biter : nullptr)) {
			done = true;
			break;
		  }
...
Status BlockBasedTable::DumpDataBlocks(WritableFile* out_file) {
  ...
  for (blockhandles_iter->SeekToFirst(); blockhandles_iter->Valid();
    ...
    out_file->Append(" @ ");
    out_file->Append(blockhandles_iter->value().ToString(true).c_str());
    // HEMANT: for debug purpose
    out_file->Append(" offset: ");
    out_file->Append(rocksdb::ToString(bh.offset()));
    out_file->Append(" size: ");
    out_file->Append(rocksdb::ToString(datablock_size));
    // HEMAND: end
    out_file->Append("\n");
    out_file->Append("--------------------------------------\n");
