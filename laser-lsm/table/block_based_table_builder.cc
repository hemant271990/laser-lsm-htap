...
struct BlockBasedTableBuilder::Rep {
  ...
  bool is_column_oriented = false; // HEMANT: to keep track if this table is column oriented
  std::vector<std::unique_ptr<BlockBasedTableBuilder>> builders; // HEMANT: vector to hold columns
  ...
  Rep(const ImmutableCFOptions& _ioptions, const MutableCFOptions& _moptions,
      ...
      const int level,  // HEMANT
      bool const _is_column_oriented) // HEMANT
      : ioptions(_ioptions),
        moptions(_moptions),
        ...
        alignment(table_options.block_align
                      ...
                      : 0),
        data_block( level >= _ioptions.column_from_k // HEMANT: do more aggresive delta encoding for column tables
                          ?  8 * table_options.block_restart_interval : table_options.block_restart_interval,
        ...
        is_column_oriented(_is_column_oriented) { // HEMANT
    ...
    if(_int_tbl_prop_collector_factories) {
		for (auto& collector_factories : *_int_tbl_prop_collector_factories) {
...
BlockBasedTableBuilder::BlockBasedTableBuilder(
    ...
    const std::string& column_family_name, const int level,  // HEMANT
    const uint64_t creation_time,
    const uint64_t oldest_key_time,
    const bool is_column_oriented) { // HEMANT
  ...
  rep_ =
      new Rep(ioptions, moptions, sanitized_table_options, internal_comparator,
              ...
              level, is_column_oriented); // HEMANT
...
void BlockBasedTableBuilder::Add(const Slice& key, const Slice& value) {
	if(rep_->is_column_oriented) {
		AddColumnOriented(key, value);
	} else {
		AddOriginal(key, value);
	}
}

void BlockBasedTableBuilder::AddOriginal(const Slice& key, const Slice& value) {
  Rep* r = rep_;
  assert(!r->closed);
  if (!ok()) return;
...
// HEMANT: begin
void BlockBasedTableBuilder::AddColumnOriented(const Slice& key, const Slice& value) {
  Rep* r = rep_;
  assert(!r->closed);
  if (!ok()) return;
  ValueType value_type = ExtractValueType(key);
  if (IsValueType(value_type)) {
	...
	//std::vector<std::string> vals(StringSplit(value_str, r->ioptions.delim.at(0)));
	std::vector<std::string> vals(StringSplitByOffset(value.data(), r->ioptions.col_attrs_offset, r->ioptions.col_attrs_size));
	uint32_t column_num = r->ioptions.column_num; //vals.size();
	if (!vals.empty() && vals.size() != column_num) {
	  r->status = Status::InvalidArgument("table_options.column_num hemant "+ToString(column_num));
	  return;
	}

	if (r->builders.empty()) {
	  r->builders.resize(column_num);
	  ...
	  for (auto i = 0u; i < column_num; i++) {
		...
		r->status = NewWritableFile(r->ioptions.env, col_fname, &file, EnvOptions());
		assert(r->status.ok());
		file->SetIOPriority(pri);
		r->builders[i].reset(new BlockBasedTableBuilder(r->ioptions, r->moptions,
			r->table_options, r->internal_comparator, nullptr, r->column_family_id,
			new WritableFileWriter(std::move(file), EnvOptions()),
			r->compression_ctx.type(), r->compression_ctx.options(), r->compression_dict,
			true, r->column_family_name, r->creation_time, r->oldest_key_time, false));
		// HEMANT: not sure what to put for skip filter and is_column_oriented
	  }
	}

	std::string val;
	PutVarint64(&val, r->props.num_entries++);
	auto should_flush = r->flush_block_policy->Update(key, Slice());
	if (should_flush) {
	  if(r->is_column_oriented){ // HEMANT: Flush columns first
		  for (auto i = 0u; i < column_num; i++) {
			const auto& rep = r->builders[i]->rep_;
			//auto should_flush_sub = rep->flush_block_policy->Update(key, vals.empty()? Slice(): vals[i]);
			//if (should_flush_sub) {
	...
	r->last_key.assign(key.data(), key.size());
	r->data_block.Add(key, Slice()); // HEMANT: TODO: for now add the whole value in the original builder
								   // so that rest of the flow
								   // doesn't break but later just add Slice() may be.
	//r->props.num_entries++;
	r->props.raw_key_size += key.size();
	r->props.raw_value_size += value.size();

	r->index_builder->OnKeyAdded(key);
	NotifyCollectTableCollectorsOnAdd(key, Slice(), r->offset,
									  r->table_properties_collectors,
									  r->ioptions.info_log);
...
Status BlockBasedTableBuilder::status() const {
  // HEMANT: if column oriented check status of all columns
  if(rep_->is_column_oriented) {
...
Status BlockBasedTableBuilder::Finish() {
  Rep* r = rep_;
  // HEMANT: if column oriented, call Finish() on all columns
  if (r->is_column_oriented) {
  ...
  if (r->is_column_oriented) {
	  for(uint32_t i = 0; i < r->builders.size(); i++){
		  if(r->builders[i] && r->builders[i]->rep_->status.ok()) {
			  r->builders[i]->rep_->file->Sync(r->ioptions.use_fsync);
			  r->builders[i]->rep_->file->Close();
			  r->builders[i].reset();
		  }
	  }
	  /*for (const auto& it : r->builders) {
...
void BlockBasedTableBuilder::Abandon() {
  ...
  if(r->is_column_oriented){
	  for(uint32_t i = 0; i < r->builders.size(); i++){
		  if(r->builders[i]) {
			assert(!r->builders[i]->rep_->closed);
			r->builders[i]->rep_->closed = true;
			r->builders[i].reset();
		  }
	  }
	  /*for (const auto& it : r->builders) {
