...
int DBImpl::FindMinimumEmptyLevelFitting(
  ...
  for (int i = level - 1; i > 0; --i) {
    ...
    if (vstorage->NumAllColumnLevelFiles(i) > 0) break;  // HEMANT
...
Status DBImpl::GetImpl(const ReadOptions& read_options,
  ...
  if (!skip_memtable) {
    if (sv->mem->Get(lkey, pinnable_val->GetSelf(), &s, &merge_context,
      ...
      PERF_COUNTER_ADD(h2saxena_get_at_mem, 1);
...
Status DBImpl::RangeQuery(const ReadOptions& read_options,
  ...
  if (range.seq != kMaxSequenceNumber)  snapshot = range.seq;
  // Acquire SuperVersion
  SuperVersion* sv = GetAndRefSuperVersion(cfd);
  Status s;
  // First look in the memtable, then in the immutable memtable (if any).
  ...
  //std::map<std::string, SeqTypeVal> tmp_res = 0;
  bool skip_memtable =
      (read_options.read_tier == kPersistedTier && has_unpersisted_data_);
  if (!skip_memtable) {
    if (!sv->mem->RangeQuery(read_options, lookup_range, res, &s/*, group, arg,
        read_options.unique_key*/)) {
      return s;
    }
    if (!sv->imm->RangeQuery(read_options, lookup_range, res, &s/*, group, arg,
        read_options.unique_key*/)) {
      return s;
    }
  }
  s = s.OK();
  sv->current->RangeQuery(read_options, lookup_range, res, &s/*, group, arg*/);
  if (!s.ok()) {
    return s;
  }
...
Status DBImpl::DeleteFile(std::string name) {
  ...
  JobContext job_context(next_job_id_.fetch_add(1), true);
  {
    ...
    for (int i = level + 1; i < cfd->NumberLevels(); i++) {
      if (vstoreage->NumAllColumnLevelFiles(i) != 0) {  // HEMANT
        ROCKS_LOG_WARN(immutable_db_options_.info_log,
    ...
    if (level == 0 &&
        vstoreage->LevelFiles(0,metadata->GetColumnNum()).back()->fd.GetNumber() != number) {  // HEMANT
      ROCKS_LOG_WARN(immutable_db_options_.info_log,
                     "DeleteFile %s failed ---"
  ...
  JobContext job_context(next_job_id_.fetch_add(1), true);
  {
    ...
    for (size_t r = 0; r < n; r++) {
      ...
      for (int i = 1; i < cfd->NumberLevels(); i++) {
    	  for(int col = 0; col < vstorage->NumLevelColumns(i); col++) {
          if (vstorage->LevelFiles(i,col).empty() ||	// HEMANT:
          ...
          vstorage->GetCleanInputsWithinInterval(
            i, begin_key, end_key, &level_files, col, -1 /* hint_index */,
            nullptr /* file_index */);
...
Status DBImpl::VerifyChecksum() {
  ...
  for (auto& sv : sv_list) {
    ...
    for (int i = 0; i < vstorage->num_non_empty_levels() && s.ok(); i++) {
      for (size_t j = 0; j < vstorage->LevelFilesBrief(i,-1).num_files && s.ok(); // HEMANT: Won't support this
           j++) {
        const auto& fd = vstorage->LevelFilesBrief(i,-1).files[j].fd;	// HEMANT: Won't support this
