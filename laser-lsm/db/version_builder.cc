...
class VersionBuilder::Rep {
 private:
  ...
  struct LevelState {
    ...
    std::unordered_map<int, std::unordered_map<uint64_t, FileMetaData*>> added_files; // HEMANT: need to bucket them by columns
  };
 ...
 public:
  ...
  ~Rep() {
    for (int level = 0; level < num_levels_; level++) {
      ...
      for (auto& col_pair : added) {
    	  for (auto& pair : col_pair.second) {
  ...
  void CheckConsistency(VersionStorageInfo* vstorage) {
    ...
    for (int level = 0; level < num_levels_; level++) {
        ...
        auto& level_files = vstorage->LevelFiles(level, col); // HEMANT
  ...
  void CheckConsistencyForDeletes(VersionEdit* /*edit*/, uint64_t number,
    ...
    for (int l = 0; !found && l < num_levels_; l++) {
    	for(int col = 0; col < base_vstorage_->NumLevelColumns(l); col++) { // HEMANT
        const std::vector<FileMetaData*>& base_files =
          base_vstorage_->LevelFiles(l,col);
    ...
    for (int l = level + 1; !found && l < num_levels_; l++) {
      ...
      for(const auto& col_pair : level_added) { // HEMANT: check in all columns
        auto got = col_pair.second.find(number);
        if (got != col_pair.second.end()) {
          found = true;
          break;
        }
      }
  ...
  void Apply(VersionEdit* edit) {
    ...
    for (const auto& del_file : del) {
      ...
      if (level < num_levels_) {
        ...
        for(auto& col_pair : levels_[level].added_files) {  // HEMANT
          auto exising = col_pair.second.find(number);
          if (exising != col_pair.second.end()) {
            UnrefFile(exising->second);
            col_pair.second.erase(exising);
            break;
          }
        }
      } else {
    ...
    for (const auto& new_file : edit->GetNewFiles()) {
      ...
      if (level < num_levels_) {
        ...
        f->refs = 1;
        int col = f->GetColumnNum(); // HEMANT
        if (levels_[level].added_files.find(col) != levels_[level].added_files.end()) {	// HEMANT: If there are enteries for this column, make sure this file is not repeated
        	assert(levels_[level].added_files[col].find(f->fd.GetNumber()) ==
        			levels_[level].added_files[col].end());
        }
        levels_[level].deleted_files.erase(f->fd.GetNumber());
        levels_[level].added_files[col].insert({f->fd.GetNumber(), f});
      } else {
  ...
  void SaveTo(VersionStorageInfo* vstorage) {
	  ...
	  for (int level = 0; level < num_levels_; level++) {
		  ...
		  for(int col = 0; col < base_vstorage_->NumLevelColumns(level); col++) {	// HEMANT: get for all columns too
			  // Merge the set of added files with the set of pre-existing files.
			  // Drop any deleted files.  Store the result in *v.
			  const auto& base_files = base_vstorage_->LevelFiles(level,col);
			  auto base_iter = base_files.begin();
			  auto base_end = base_files.end();
			  //vstorage->Reserve(level, base_files.size() + unordered_added_files.size()); // HEMANT: we can avoid this.

			  // Sort added files for the level.
			  std::vector<FileMetaData*> added_files;
			  if (levels_[level].added_files.find(col) != levels_[level].added_files.end())	// HEMANT: check if added files present for this col
			  {
				  const auto& unordered_added_files = levels_[level].added_files[col];
  ...
  void LoadTableHandlers(InternalStats* internal_stats, int max_threads,
    ...
    for (int level = 0; level < num_levels_; level++) {
    	for(auto& col_pair : levels_[level].added_files) {
        for (auto& file_meta_pair : col_pair.second) { // HEMANT: open it for all columns
