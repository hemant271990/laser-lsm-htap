...
Status CompactedDBImpl::Get(const ReadOptions& options, ColumnFamilyHandle*,
  ...
  GetContext get_context(user_comparator_, nullptr, nullptr, nullptr,
                         ...
                         nullptr, nullptr, 0,0,'|', {}, nullptr); // HEMANT: this is not supported, pass required_columns=0,column_num=0,delim='|'
  ...
  for (auto* r : reader_list) {
    if (r != nullptr) {
      ...
      GetContext get_context(user_comparator_, nullptr, nullptr, nullptr,
                             ...
                             nullptr, nullptr, nullptr, nullptr, 0,0,'|', {}, nullptr);  // HEMANT: this is not supported, pass required_columns=0,column_num=0,delim='|'
...
Status CompactedDBImpl::Init(const Options& options) {
  ...
  // HEMANT: This might not be executed. If it is, just checking col=0 should be enough
  const LevelFilesBrief& l0 = vstorage->LevelFilesBrief(0,0);
  ...
  for (int i = 1; i < vstorage->num_non_empty_levels() - 1; ++i) {
    if (vstorage->NumAllColumnLevelFilesBrief(i) > 0) {	// HEMANT
      return Status::NotSupported("Other levels also contain files");
    }
  }

  int level = vstorage->num_non_empty_levels() - 1;
  if (vstorage->NumAllColumnLevelFilesBrief(level) > 0) { // HEMANT
    files_ = vstorage->LevelFilesBrief(level, -1);  // HEMANT
    return Status::OK();
  }
  return Status::NotSupported("no file exists");
}
