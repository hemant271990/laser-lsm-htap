...
ImmutableMemTableOptions::ImmutableMemTableOptions(
    ...
    : arena_block_size(mutable_cf_options.arena_block_size),
      ...
      delim(ioptions.delim), // HEMANT
      col_attrs_offset(ioptions.col_attrs_offset), // HEMANT
      col_attrs_size(ioptions.col_attrs_size){} // HEMANT
...
struct Saver {
  Status* status;
  const LookupKey* key;
  const LookupRange* range;  // HEMANT
  bool* found_final_value;  // Is value set correctly? Used by KeyMayExist
  bool* merge_in_progress;
  std::string* value;
  /*std::map<std::string, SeqTypeVal>**/int* res;  // HEMANT
  SequenceNumber seq;
  ...
  Env* env_;
  filterFun filter;  // HEMANT
  groupFun group;    // HEMANT
  void* arg;         // HEMANT
  bool unique_key;   // HEMANT
...
static bool SaveValueForRangeQuery(void* arg, const char* entry,
									std::vector<uint32_t> cols_index,
    ...
    switch (type) {
      ...
      case kTypeSingleDeletion: {
        if (s->seq <= sequence_num) {
		  ...
		  std::vector<std::string> vals(StringSplit(val, s->mem->GetImmutableMemTableOptions()->delim.at(0)));
		  for(uint32_t j = 0; j < cols_index.size(); j++) {
			v[cols_index[j]] = &vals[cols_index[j]-1];
		  }
		  if (!s->filter || (s->filter && s->filter(v))) {
			  s->res->emplace(user_key, SeqTypeVal(s->seq, type, val));;
		  }*/

		  Slice value_slice = GetLengthPrefixedSlice(key_ptr + key_length);
		  if(filter_values.size() > 0) {
			bool filter_pass = true;
			for(uint32_t j = 0; j < cols_index.size(); j++) {
				uint32_t col_index = cols_index[j]-1;
				size_t attr_size = s->mem->GetImmutableMemTableOptions()->col_attrs_size[col_index];
				size_t attr_offset = s->mem->GetImmutableMemTableOptions()->col_attrs_offset[col_index];
				char filter_type = filter_types[j];
				char c_val[attr_size];
				memcpy(c_val, &value_slice.data_[attr_offset], attr_size);
				switch(filter_type) {
					case 'D' :
					{
					  double filter = std::stod(filter_values[j]);
					  double value = std::atof(c_val);
					  if(filter_comparator[j] == ">") {
						  if(value <= filter)
							  filter_pass = false;
					  }
					  break;
...
/***************************** HEMANT *****************************/
bool MemTable::RangeQuery(const ReadOptions& options,
  ...
  if (range.SequenceNum() < GetFirstSequenceNumber()) {
    *s = Status::NotFound(Slice());
    return true;
  }
  ...
  table_->RangeQuery(range, /*res,*/ &saver,
		  	  	  	  options.filter_columns,
		  	  	  	  options.filter_types,
		  	  	  	  options.filter_values,
		  	  	  	  options.filter_comparator,
		  	  	  	  SaveValueForRangeQuery);
