...
void DBImpl::NotifyOnFlushBegin(ColumnFamilyData* cfd, FileMetaData* file_meta,
  ...
  bool triggered_writes_slowdown =
      (cfd->current()->storage_info()->NumAllColumnLevelFiles(0) >= // HEMANT
...
Status DBImpl::ReFitLevel(ColumnFamilyData* cfd, int level, int target_level) {
  ...
  if (to_level != level) {
    ...
    // HEMANT: move all column files
    for(int col = 0; col < vstorage->NumLevelColumns(level); col++) {
      for (const auto& f : vstorage->LevelFiles(level,col)) {
        edit.DeleteFile(level, f->fd.GetNumber());
        edit.AddFile(to_level, f->fd.GetNumber(), f->fd.GetPathId(),
              f->fd.GetFileSize(), f->smallest, f->largest,
              f->smallest_seqno, f->largest_seqno,
              f->marked_for_compaction, f->column_file);
      }
    }
    ROCKS_LOG_DEBUG(immutable_db_options_.info_log,
...
void DBImpl::MaybeScheduleFlushOrCompaction() {
  ...
  while (bg_compaction_scheduled_ < bg_job_limits.max_compactions &&
    ...
    ROCKS_LOG_INFO(immutable_db_options_.info_log,
                     "[HEMANT] Scheduling compaction # unscheduled compactions %d, # compaction scheduled %d max_compaction %d",
                      unscheduled_compactions_, bg_compaction_scheduled_, bg_job_limits.max_compactions);
    env_->Schedule(&DBImpl::BGWorkCompaction, ca, Env::Priority::LOW, this,
                   &DBImpl::UnscheduleCallback);
  }
}
...
void DBImpl::SchedulePendingCompaction(ColumnFamilyData* cfd) {
  if (!cfd->queued_for_compaction() && cfd->NeedsCompaction()) {
    ...
    ++unscheduled_compactions_;
    ROCKS_LOG_INFO(immutable_db_options_.info_log,
                     "[HEMANT] add to compaction queue # unscheduled compactions %d, # compaction scheduled %d", unscheduled_compactions_, bg_compaction_scheduled_);
  }
}
