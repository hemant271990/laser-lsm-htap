...
Status ExternalSstFileIngestionJob::Run() {
  ...
  for (IngestedFileInfo& f : files_to_ingest_) {
    ...
    edit_.AddFile(f.picked_level, f.fd.GetNumber(), f.fd.GetPathId(),
                  ...
                  false, false );	// HEMANT: Won't support this. Pass column_file = false
  }
...
Status ExternalSstFileIngestionJob::AssignLevelAndSeqnoForIngestedFile(
  ...
  *assigned_seqno = 0;
  ...
  for (int lvl = 0; lvl < cfd_->NumberLevels(); lvl++) {
    ...
    if (vstorage->NumAllColumnLevelFiles(lvl) > 0) { // HEMANT
      ...
      if (compaction_style == kCompactionStyleUniversal && lvl != 0) {
        const std::vector<FileMetaData*>& level_files =
            vstorage->LevelFiles(lvl,-1);	// HEMANT: we dont support it.
...
Status ExternalSstFileIngestionJob::CheckLevelForIngestedBehindFile(
  ...
  for (int lvl = 0; lvl < cfd_->NumberLevels() - 1; lvl++) {
    for (auto file : vstorage->LevelFiles(lvl,-1)) {	// HEMANT: we dont support it
