...
Status TableCache::GetTableReader(
    ...
    const std::vector<uint32_t>& cols, bool os_cache) { // HEMANT
  std::string fname =
  ...
  Status s = ioptions_.env->NewRandomAccessFile(fname, &file, os_cache? env_options: eo); // HEMANT
  ...
  if (s.ok()) {
    ...
    s = ioptions_.table_factory->NewTableReader(
        TableReaderOptions(ioptions_, prefix_extractor, env_options,
                           internal_comparator, skip_filters, level, cols),
...
InternalIterator* TableCache::NewIterator(
  ...
  if (for_compaction) {
    ...
    if (!os_cache) {  // when for_compaction && !os_cache true, it is range query
          readahead = options.readahead_size;  // HEMANT
          create_new_table_reader = true;      // HEMANT
  ...
  if (create_new_table_reader) {
    ...
    s = GetTableReader(
        ...
        true /* prefetch_index_and_filter_in_cache */, for_compaction, options.filter_columns, os_cache);
    if (s.ok()) {
