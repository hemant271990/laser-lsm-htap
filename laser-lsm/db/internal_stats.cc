...
bool InternalStats::HandleNumFilesAtLevel(std::string* value, Slice suffix) {
  ...
  if (!ok || static_cast<int>(level) >= number_levels_) {
    ...
    snprintf(buf, sizeof(buf), "%d",
             vstorage->NumAllColumnLevelFiles(static_cast<int>(level))); // HEMANT
    *value = buf;
    return true;
  }
}
...
bool InternalStats::HandleLevelStats(std::string* value, Slice /*suffix*/) {
  ...
  for (int level = 0; level < number_levels_; level++) {
    snprintf(buf, sizeof(buf), "%3d %8d %8.0f\n", level,
             vstorage->NumAllColumnLevelFiles(level),	// HEMANT
...
void InternalStats::DumpCFMapStats(
  ...
  for (int level = 0; level < number_levels_; ++level) {
	  for(int col = 0; col < vstorage->NumLevelColumns(level); col++) { // HEMANT: get for all columns
		for (auto* f : vstorage->LevelFiles(level, col)) {
  ...
  for (int level = 0; level < number_levels_; level++) {
    int files = vstorage->NumAllColumnLevelFiles(level);
    total_files += files;
