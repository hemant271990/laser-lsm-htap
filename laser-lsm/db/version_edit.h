...
struct FileMetaData {
  ...
  bool column_file;				// HEMANT: column file
  ...
  // HEMANT: get column num for meta data
  int GetColumnNum() {
    // UPDATE: info whether it is a column_file or not, is not really important
	  /*if (!column_file) {
		  return 0;
	  }*/
	  int f_num = fd.GetNumber();
	  int col = (int)f_num/1000000;
    if(col == 0) {
      return 0;
    } else {
	    return col-1;
...
class VersionEdit {
 public:
  ...
  void AddFile(int level, uint64_t file, uint32_t file_path_id,
               ...
               bool marked_for_compaction, bool column_file) {	// HEMANT
    ...
    f.column_file = column_file;	// HEMANT
