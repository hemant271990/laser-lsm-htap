...
class VersionStorageInfo {
 public:
  VersionStorageInfo(const InternalKeyComparator* internal_comparator,
                     ...
                     bool _force_consistency_checks, int column_from_k, 
                     const std::vector<uint32_t>& levels_cg_count,
  ...
  // HEMANT: (moved it here) used to sort files by size
  struct Fsize {
    size_t index;
    FileMetaData* file;
  };

  // HEMANT: new struct. used to sort by # files in a column
  struct ColSize {
    size_t index;
    size_t num_columns;
    FileMetaData* file;
  ...
  bool OverlapInLevel(int level, const Slice* smallest_user_key,
                      const Slice* largest_user_key, int col = -1);
  ...
  // REQUIRES: This version has been saved (see VersionSet::SaveTo)
  // HEMANT: make it return num for specific column
  int NumLevelFiles(int level, int column) const {
    assert(finalized_);
    assert(column >= 0);
    if(files_[level].empty())
    	return 0;
    if(column < static_cast<int>(files_[level].size())) {
      return static_cast<int>(files_[level][column].size());
    } else {
      return 0;
    }
  }

  // HEMANT: Return num columns at each level
  int NumLevelColumns(int level) const {
	  //assert(finalized_);
	  if(files_[level].empty())
		  return 0;
  ...
  // HEMANT
  int NumAllColumnLevelFiles(int level) const {
	  if(files_[level].empty())
		  return 0;
	  int count = 0;
	  for(int col = 0; col < static_cast<int>(files_[level].size()); col++) {
		  count += files_[level][col].size();
	  }
	  return count;
  }
  ...
  // REQUIRES: This version has been saved (see VersionSet::SaveTo)
  // HEMANT: level files for a column
  const std::vector<FileMetaData*>& LevelFiles(int level, int col) const {
	assert(!files_[level].empty()); // HEMANT: make sure column exist
	assert(col >= 0);
  assert(col < static_cast<int>(files_[level].size()));
  return files_[level][col];
  }

  const rocksdb::LevelFilesBrief& LevelFilesBrief(int level, int col) const {	// HEMANT
    assert(level < static_cast<int>(level_files_brief_.size()));
    assert(col >= 0);
    assert(col < static_cast<int>(level_files_brief_[level].size()));
    return level_files_brief_[level][col];
  }

  int NumAllColumnLevelFilesBrief(int level) const {	// HEMANT
      assert(level < static_cast<int>(level_files_brief_.size()));
      int count = 0;
      for(int col = 0; col < static_cast<int>(level_files_brief_[level].size()); col++)	{
    	  count += static_cast<int>(level_files_brief_[level][col].num_files);
      }
      return count;
    }

  // REQUIRES: This version has been saved (see VersionSet::SaveTo)
  const std::vector<Fsize>& FilesByCompactionPri(int level) const { // HEMANT: changed to Fsize from int
    assert(finalized_);
  ...
  // HEMANT
  int column_from_k() const { return column_from_k_; }
  //int num_column() const { return column_num_; }
  std::vector<uint32_t> levels_cg_count() const { return levels_cg_count_; }
  std::vector<std::vector<std::tuple<uint32_t, uint32_t>>> cg_range_matrix() const { return cg_range_matrix_; }
  ...
  bool RangeMightExistAfterSortedRun(const Slice& smallest_key,
                                     ...
                                     int last_l0_idx, int col);

 private:
  const InternalKeyComparator* internal_comparator_;
  const Comparator* user_comparator_;
  int num_levels_;            // Number of levels
  int column_from_k_;         // HEMANT: level k onwards is column oriented
  //int column_num_;			  // HEMANT: number of columns in column oriented
  std::vector<uint32_t> levels_cg_count_; // HEMANT: number of CGs per level
  std::vector<std::vector<std::tuple<uint32_t, uint32_t>>> cg_range_matrix_; // CG matrix with range (inclusive)
  ...
  autovector<autovector<rocksdb::LevelFilesBrief>> level_files_brief_;	// HEMANT
  ...
  // HEMANT: modifications to above files_. This list of files per level segregates
  // each level by columns. Level < k have only one column indexed at 0.
  // levels >=k entry 0,1,2,... corresponds to columns 1,2,3...
  ...
  std::vector<std::vector<Fsize>> files_by_compaction_pri_; // HEMANT: store FileMetaData too. Will help to extract column
...
class VersionSet {
 public:
  ...
  uint64_t ApproximateSize(Version* v, const Slice& start, const Slice& end,
                           int start_level = 0, int end_level = -1, int col = -1); // HEMANT
