...
static std::string MakeFileName(const std::string& name, uint64_t number,
                                const char* suffix) {
  char buf[100];
  snprintf(buf, sizeof(buf), "/%09llu.%s",
           static_cast<unsigned long long>(number),
           suffix); // HEMANT: 9 digits, most significant 3 digits for column number starting from 1
  return name + buf;
}
...
int TableFileNameToColumnNumber(const std::string& name) {
  uint64_t number = 0;
  uint64_t base = 1;
  int pos = static_cast<int>(name.find_last_of('.'));
  while (--pos >= 0 && name[pos] >= '0' && name[pos] <= '9') {
  ...
  // HEMANT: we return 0 for non-column files, and >0 as column num for column files
  // Caller should substract 1 from col_num if it has to be used for zero-based index
  int col = (int)number/1000000;
	return col;
}
...
std::string TableSubFileName(const std::string& name, uint32_t number) {
  ...
  const std::string ext(".sst");
  if ( name != ext &&
       name.size() > ext.size() &&
       name.substr(name.size() - ext.size()) == ".sst" )
  {
     // if so then strip them off
	 std::string stripped = name.substr(0, name.size() - ext.size());
     return stripped + buf + ext;
  } else {
	  return name;
  }
}
