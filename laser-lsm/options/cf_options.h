...
struct ImmutableCFOptions {
  ...
  // HEMANT: # of columns in which the column oriented levels are broken
  uint32_t column_num;

  // HEMANT: delimiter for columns
  std::string delim;
  ...
  // HEMANT: for column groups (CG)
  std::vector<std::vector<std::tuple<uint32_t, uint32_t>>> cg_range_matrix= {}; // CG matrix with range (inclusive)
  std::vector<uint32_t> levels_cg_count = {};  // number of CGs per level
