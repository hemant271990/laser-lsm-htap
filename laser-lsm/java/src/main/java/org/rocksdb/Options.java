...
public class Options extends RocksObject
  ...
  // HEMANT begin
  @Override
  public Options setColumnFromK(final int flag) {
    assert(isOwningHandle());
    setColumnFromK(nativeHandle_, flag);
    return this;
  }

  @Override
  public int columnFromK() {
    assert(isOwningHandle());
    return columnFromK(nativeHandle_);
  }
  @Override
  public Options setDelim(final String flag) {
    assert(isOwningHandle());
    setDelim(nativeHandle_, flag);
    return this;
  }
  ...
  public String delim() {
    assert(isOwningHandle());
    return delim(nativeHandle_);
  }
  
  @Override
  public Options setColumnNum(final int flag) {
    assert(isOwningHandle());
    setColumnNum(nativeHandle_, flag);
    return this;
  }
  ...
  public int columnNum() {
    assert(isOwningHandle());
    return columnNum(nativeHandle_);
  }
  
  @Override
  public Options setColAttrsOffset(final int[] offsets) {
    assert(isOwningHandle());

    final int len = offsets.length;
    final int off[] = new int[len];

    for(int i = 0; i < len; i++) {
      off[i] = offsets[i];
    }
    setColAttrsOffset(nativeHandle_, off);
    return this;
  }
  
  @Override
  public Options setColAttrsSize(final int[] offsets) {
    assert(isOwningHandle());
    ...
    setColAttrsSize(nativeHandle_, off);
    return this;
  }
  // HEMANT end
  ...
  // HEMANT begin
  private native void setColumnFromK(long handle, int flag);
  private native int columnFromK(long handle);
  private native void setDelim(long handle, String flag);
  private native String delim(long handle);
  private native void setColumnNum(long handle, int flag);
  private native int columnNum(long handle);
  private native void setColAttrsOffset(long handle, int[] offsets);
  private native void setColAttrsSize(long handle, int[] offsets);
  // HEMANT end
