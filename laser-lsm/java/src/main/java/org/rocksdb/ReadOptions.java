...
public class ReadOptions extends RocksObject {
  ...
  // HEMANT begin
  public ReadOptions setFilterColumns(final int[] offsets) {
    assert(isOwningHandle());
    ...
    setFilterColumns(nativeHandle_, off);
    return this;
  }
  
  public ReadOptions setFilterTypes(final char[] offsets) {
    assert(isOwningHandle());
    
    final int len = offsets.length;
    final char off[] = new char[len];

    for(int i = 0; i < len; i++) {
      off[i] = offsets[i];
    }
    setFilterTypes(nativeHandle_, off);
    return this;
  }
  
  public ReadOptions setFilterValues(final String[] offsets) {
    assert(isOwningHandle());
    
    final int len = offsets.length;
    final String off[] = new String[len];

    for(int i = 0; i < len; i++) {
      off[i] = offsets[i];
    }
    setFilterValues(nativeHandle_, off);
    return this;
  }
  
  public ReadOptions setFilterComparator(final String[] offsets) {
    assert(isOwningHandle());
    ...
    setFilterComparator(nativeHandle_, off);
    return this;
  }
  // HEMANT end
  ...
  // HEMANT begin
  private native void setFilterColumns(final long handle, final int[] offsets);
  private native void setFilterTypes(final long handle, final char[] offsets);
  private native void setFilterValues(final long handle, final String[] offsets);
  private native void setFilterComparator(final long handle, final String[] offsets);
  // HEMANT end
