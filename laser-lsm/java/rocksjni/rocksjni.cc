 ...
 * Signature: (JJ[BIIJ)[B
 */
jint Java_org_rocksdb_RocksDB_rangeQuery(
    JNIEnv* env, jobject /*jdb*/, jlong jdb_handle, jlong jropt_handle,
    jstring jbegin, jstring jend) {
  auto* db_handle = reinterpret_cast<rocksdb::DB*>(jdb_handle);
  auto& ro_opt = *reinterpret_cast<rocksdb::ReadOptions*>(jropt_handle);
  jboolean has_exception = JNI_FALSE;
  int sum = 0;
  std::string begin = rocksdb::JniUtil::copyStdString(env, jbegin, &has_exception);
  std::string end = rocksdb::JniUtil::copyStdString(env, jend, &has_exception);
  db_handle->RangeQuery(ro_opt, rocksdb::Range(begin, end), &sum);
  return sum;
}
