...
vector<vector<tuple<uint32_t, uint32_t>>> createCGMatrix19(int num_levels, uint32_t column_num) {
    ...
    vector<tuple<uint32_t, uint32_t>> cg_j4{make_tuple(0,14), make_tuple(15,19), make_tuple(20,24), make_tuple(25,27), make_tuple(28,29)};
...
DB* initDB() {
	...
	options.write_buffer_size = 16 * 1024 * 1024;	// default 64MB
	...
	options.max_bytes_for_level_base = 1 * 32 * 1024 * 1024ul; // default 256MB
	...
	if (design == 7) {
		...
		options.cg_range_matrix = createCGMatrix19(max_levels, column_count);
                options.levels_cg_count = {1, 1, 2, 2, 3, 3, 5, 5};
	} else if (design == -1) {
...
uint32_t getNextReadKeyNormal() {
	std::normal_distribution<> norm_dist{norm_mean,0.02};
...
uint32_t getNextScanKey() {
	...
	//uint64_t curr_key = key_counter.load() - 100;
	//uint32_t user_key = (uint32_t) (rand() % curr_key + 1);
	uint32_t rand_num = (uint32_t) (rand() % 15 + 1);
...
void read(DB* db) {
	string kstr = getKeyStr(getNextReadKey());
...
void scan(DB* db) {
	...
	ReadOptions ro;
	ro.filter_columns = scan_filter;
	Iterator *it = db->NewIterator(ro);
	auto start = std::chrono::high_resolution_clock::now();
	it->Seek(kstr);
	//it->SeekToFirst();
	uint64_t scan_amount = static_cast<uint64_t>(scan_length*key_counter.load());
	cout << "Column: " << scan_filter[0] << " Scanning for " << scan_amount << " entries out of " << key_counter.load() << " from key: " << kstr << endl;
...
void insert(DB* db) {
	...
	while(operation_counter.load() < num_operations) {
		string kstr = getInsertKeyStr();
		std::ostringstream vstr;
		for (uint32_t i = 0; i < column_count; i++) {
			int v = rand() % 1000 + 1;
...
void runWorkload(DB* db) {
	...
	while( curr_num_operations < num_operations) {
		...
		if(curr_num_operations % 250 == 0) {
			cout << "sleeping every 500 ops" << endl;
...
int main(int argc, char** argv) {
	...
	if(mode == "load") {
		...
		if(inserts_with_reads) {
			...
			cout << "Give inserts a head start. Sleeping for (secs):" << 1200 << endl;
		...
		cout << "Sleeping for (secs):" << 7200 << endl;
		this_thread::sleep_for(chrono::seconds(7200));
	} else if(mode == "update") {
